    ...
    if not session.get("email"):
        ...
        if form.validate_on_submit():
            ...
            if (
                ...
                and (bcrypt.checkpw(form.password.data.encode("utf-8"), temp["pwd"]))
            ):
                flash("You have been logged in!", "success")
                session["email"] = temp["email"]
                session["name"] = temp["name"]
                # session['login_type'] = form.type.data
                data = mongo.profile.find_one(
                    {"email": temp["email"]}, {"weight", "height", "target_weight"}
                )
                if data:
                    return redirect(url_for("dashboard"))
                else:
                    ...
                    )  # render_template('user_profile.html', title = "Details")
            else:
                flash(
...
def display_profile():
    ...
    if session.get("email"):
        email = session.get("email")
        data = mongo.profile.find_one(
            {"email": email}, {"weight", "height", "target_weight"}
        )
        return render_template(
            "display_profile.html", title="Profile", data=data, status=True
        )
    ...
    if session.get("email"):
        ...
        if form.validate_on_submit():
            ...
            if request.method == "POST":
                ...
                temp = mongo.profile.find_one(
                    {"email": email}, {"height", "weight", "goal", "target_weight"}
                )
                if temp is not None:
                    mongo.profile.update_one(
                        {"email": email},
                        {
                            "$set": {
                                "weight": weight,
            ...
            return render_template(
                "display_profile.html", data=data, status=True, form=form
            )
    else:
        return redirect(url_for("login"))
...
def friends():
    ...
    if (
        sendingemail
        and sendingemail not in pendingReceivers
        and sendingemail not in pendingApproves
        and sendingemail not in myFriendsList
    ):
...
def send_email():
    ...
    email = session.get("email")
    data = list(
        mongo.calories.find({"email": email}, {"date", "email", "calories", "burnout"})
    )
...
def ajaxcancelrequest():
    ...
    if get_session is not None:
        receiver = request.form.get("receiver")
        res = mongo.friends.delete_one({"sender": email, "receiver": receiver})
        if res:
            return (
                json.dumps({"status": True}),
                200,
...
def yoga():
    ...
    if get_session is not None:
        ...
        if form.validate_on_submit():
            if request.method == "POST":
                enroll = "yoga"
                status = mongo.user.find_one(
                    {"Email": email, "Status": enroll}, {"Email", "Status"}
                )
                print(status)
                if status:
                    flash(f"You have already enrolled in our {enroll} plan!", "warning")
                else:
...
def workout_suggestions():
    ...
    result = mongo.profile.find_one(
        {"email": email}, {"height", "weight", "target_weight"}
    )
    ...
    if bmi < 18.5:
        ...
        suggestions.append(
            "To gain weight: Focus on strength training and calorie surplus."
        )
        ...
        suggestions.append(
            "To maintain weight: Maintain a balanced workout routine and calorie intake."
        )
        ...
        suggestions.append(
            "Exercise 4: Dumbbell Rows - 3 sets of 10 reps (if available)"
        )
        ...
        suggestions.append(
            "To lose weight: Focus on cardio and maintain a calorie deficit."
        )
        # Include specific workout plans for Overweight

        # High-Intensity Interval Training (HIIT)
        suggestions.append(
            "Week 1-2 (3 days a week): High-Intensity Interval Training (HIIT)"
        )
        ...
        suggestions.append(
            "Exercise 3: Mountain Climbers - 45 seconds, Rest 15 seconds"
        )
    ...
    return render_template(
        "suggestion.html", title="Suggestion", status=True, data=suggestions
    )
